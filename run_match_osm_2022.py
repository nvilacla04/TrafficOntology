"""
Script to match BRON accident data for 2022 to OSM road segments.

This script reads a cleaned BRON accidents CSV for the year 2022, converts it to a
GeoDataFrame, and spatially joins each accident point to the nearest OSM road
segment sharing the same street name.  Selected attributes from the road
...
def main() -> None:
    # -------------------------------------------------------------------------
    # Step 1: Load the BRON accident data for 2022
    # -------------------------------------------------------------------------
    # Path to the cleaned 2022 accidents CSV file.  Use a raw string for
    # Windows-style paths to avoid needing to escape backslashes.
    ...
    print(f"Loaded {len(df_bron)} accidents with valid locations and street names from 2022.")
    ...
    # -------------------------------------------------------------------------
    print("\n--- 2. PREPARING DATA ---")
    # Step 2a: Convert the BRON DataFrame into a GeoDataFrame
    ...
    # Step 2c: Parse the 'other_tags' column to extract useful attributes
